...
extension Date {
    ...
    func endOfDay() -> Date {
        let calendar = Calendar.current
        return calendar.date(byAdding: .minute, value: -1, to: calendar.date(byAdding: .day, value: 1, to: self.startOfDay())!)!
    }
    
    func isMoreThanDayAgo() -> Bool {
        let date1 = Date()
        let date2 = self
        if date1.daysBetween(date: date2) >= 1 {
            return true
        }
        return false
    }
